...
data_folder = "/home/pi/.signalk/plugin-config-data"
file_to_open = data_folder + "/sk-plugin-ozzberryimu.json"
...
config = SK_Plugin_config['configuration']
...
##################### BMP388 Code ######################################
    ...
    if config["Heading"]:                       # Change to 'false' to stop sending the heading
        skDataTx = '{"updates": [{ "values": [{ "path": "navigation.headingCompass", "value" :%01.3f' % (math.radians(tiltCompensatedHeading)) + ' }] }] }'
        serial_dict = json.loads(skDataTx.encode('ascii', 'strict'))
        sys.__stdout__.write(json.dumps(serial_dict))
        sys.__stdout__.write('\n')
        sys.__stdout__.flush()
        time.sleep(0.1)

    if config["RateOfTurn"]:                       # Change to '0' to stop sending the rate of turn
        skDataTx = '{"updates": [{ "values": [{ "path": "navigation.rateOfTurn", "value": %1.0f' % (math.radians(rateOfTurn)) + ' }] }] }'
        serial_dict = json.loads(skDataTx.encode('ascii', 'strict'))
        sys.__stdout__.write(json.dumps(serial_dict))
        sys.__stdout__.write('\n')
        sys.__stdout__.flush()
    ...
    if config["PitchRoll"]:                       # Change to '0' to stop sending the roll/pith angles from the Kalman filters
        skDataTx = '{"updates": [{ "values": [{ "path": "navigation.attitude", "value": { "roll": %01.3f' % (math.radians(kalmanX)) + ', "pitch": %4.3f' % (math.radians(kalmanY)) + ' } }] }] }'
        serial_dict = json.loads(skDataTx.encode('ascii', 'strict'))
        sys.__stdout__.write(json.dumps(serial_dict))
        sys.__stdout__.write('\n')
        sys.__stdout__.flush()
    ...
    if config["Temp"]:                       # Change to '0' to stop sending the temperature inside the RPi enclosure
        skDataTx = '{"updates": [{ "values": [{ "path": "environment.rpi.temperature", "value": %01.1f' % (temp) + ' }] }] }'
        serial_dict = json.loads(skDataTx.encode('ascii', 'strict'))
        sys.__stdout__.write(json.dumps(serial_dict))
        sys.__stdout__.write('\n')
        sys.__stdout__.flush()
    ...
    if config["Barometer"]:                       # Change to '0' to stop sending the Barometric pressure from inside the RPi enclosure
        skDataTx = '{"updates": [{ "values": [{ "path": "environment.rpi.pressure", "value": %01.2f' % (pres) + ' }] }] }'
        serial_dict = json.loads(skDataTx.encode('ascii', 'strict'))
        sys.__stdout__.write(json.dumps(serial_dict))
        sys.__stdout__.write('\n')
        sys.__stdout__.flush()
    ...
    if config["Altitude"]:                       # Change to '0' to stop sending the Altitude of the RPi enclosure
        skDataTx = '{"updates": [{ "values": [{ "path": "environment.rpi.altitude", "value": %01.1f' % (altitude) + ' }] }] }'
        serial_dict = json.loads(skDataTx.encode('ascii', 'strict'))
        sys.__stdout__.write(json.dumps(serial_dict))
        sys.__stdout__.write('\n')
        sys.__stdout__.flush()
